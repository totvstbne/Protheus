...
Static Function ProcINI(oProcess)
	...
	WHILE !TCOMP->(EOF())
		IF cTipo == "Cadastro"
			...
			cQuery += " AND    SUBSTRING(SRA.RA_BCDEPSA,1,3) = '032'
			...
			if TSRA->(EOF())
				Alert("Nenhum funcionário foi localizado com os parâmetros informados !")
			...
			WHILE !TSRA->(EOF())
				...
				IF cFilt  <> TSRA->RA_FILIAL
					...
					nHdl := fCreate(cArqOut+"APT_ARQ_CAD_MITTU"+ "00000" + DTOS(DDATABASE) + STRTRAN(TIME(), ":", "") + '.txt')
				...
				*/
				...
				cTexto += PADR(SUBSTR(TSRA->RA_NASC,7,2) + SUBSTR(TSRA->RA_NASC,5,2) + SUBSTR(TSRA->RA_NASC,1,4),8,"")
				...
				ENDIF 
				//Data Emissão do RG
				cTexto += PADR(SUBSTR(TSRA->RA_DTRGEXP,7,2) + SUBSTR(TSRA->RA_DTRGEXP,5,2) + SUBSTR(TSRA->RA_DTRGEXP,1,4),8,"")
				...
				cTexto += PADR(SUBSTR(TSRA->RA_ADMISSA,7,2) + SUBSTR(TSRA->RA_ADMISSA,5,2) + SUBSTR(TSRA->RA_ADMISSA,1,4),8,"")
				//cTexto += PADR(TSRA->RA_ADMISSA,8,"")
				//Código do cartão --> NOVO LAYOUT "CAMPO LIVRE"
				cTexto += PADR(ALLTRIM(TSRA->RA_CC),16,"")
				//Número da linha
				cTexto += PADL(nLinha,5,"0")
				//Retorno do processamento
				cTexto += PADR("",2,"")
				...
				*/
				...
				//Localidade
				cTexto += PADR(ALLTRIM(POSICIONE("CC2",1,xFilial("CC2",TSRA->RA_MAT) + TSRA->RA_ESTADO + TSRA->RA_CODMUN,"CC2_MUN")),40,"")
				//Unidade Federativa
				...
				*/
				...
				// caso não tenha telefone não gerar o seguimento 33
				// conforme orientação 22/08/19

				if alltrim(TSRA->RA_TELEFON) <> "" .AND. ALLTRIM(TSRA->RA_DDDFONE) <> ""
					...
					//Número Telefone
					cTexto += PADL(TSRA->RA_TELEFON,10,"0")
					//Número da linha
					cTexto += PADL(nLinha,5,"0")
					//Retorno do processamento
					cTexto += PADR("",2,"")
				...
				if TCOMP->RCH_PERSEL == "1" // periodo em aberto
					...
					WHILE !TSRA->(EOF())
						...
						IF cFilt  <> TSRA->RA_FILIAL
							// CRIAR AQUIVO 
							cFilt := TSRA->RA_FILIAL
							nHdl := fCreate(cArqOut+"APT_ARQ_DEP_MITTU"+ "00000" + DTOS(DDATABASE) + STRTRAN(TIME(), ":", "") + '.txt')
							...
							// CRIAR ARQUIVO LOG
							cFilt := TSRA->RA_FILIAL
							nHdlLog := fCreate(cArqOut+"RELATORIO_DEP_MITTU"+ cFilt + DTOS(DDATABASE) + STRTRAN(TIME(), ":", "") + '.txt')
						...
						*/
						...
						cTexto += PADr(ALLTRIM(TSRA->RA_CIC),14,"")
...
Static Function FMULSA1S(cTitulo,lTipoRet)
...
// CONTRATOS 
Static Function FMULCN9S(cTitulo,lTipoRet)
...
Static Function FMULSRAS(cTitulo,lTipoRet)
